...
const createNewStudent = asyncHandler(async (req, res) => {
  const { name, course, email, username,semester, password } = req.body;

  // Confirm Data
  if (!name || !email || !course || !username || !password|| !semester) {
...
const updateStudent = asyncHandler(async (req, res) => {
  const { id, name, email, username, password,semester } = req.body;

  // Confirm Data
  if (!id || !name || !email || !username|| !semester) {
    return res.status(400).json({ message: "All fields are required" });
  }
